import { geminiEngine } from "../engine/ai_engine.js";
import { saveExtraction } from "../services/task.service.js";
import { OperationFailedError } from "../response_handler/exceptions.js";
...
const processTextContent = async (text) => {
    ...
    const cleanText = text
        ...
        .trim();
    if (!cleanText)
        throw new Error("Processed text is empty!");
    return cleanText;
};
const processPdfContent = async (buffer) => {
...
const processIngestion = async (format, content, userId, fileName, mimeType) => {
    let extractedText;
    let extraction;
    try {
        if (format === 'pdf') {
